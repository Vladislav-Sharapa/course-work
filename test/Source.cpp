...
void inputStudentList(bool*, int&, student*&, short int); //добавление данных в массив (сделано)
...
void sortStudentList(int, student*&); //сортировка массива данных

void outputStudentList(student*, int); //просмотр списка (сделана)
...
void inputStudentList(bool* pCondition, int& size, student*& pMassive, short int action) {
	...
	case inputAction::accommodation_of_student:  // заселение студентов 
		...
		for (int i = 0; i < size; i++) {
			...
			cout << "Занимался ли студент социальными работами({Да} или {Нет}): ";
